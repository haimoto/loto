...
v5.12（2026-08-01）の変更:
- 拡張プラス（11〜20口）を組数ごとに独立再探索し、16ケースの any3 を改善。
  loto7 12〜20口・loto6 14〜20口。最大は loto7 20口 96.6473%→97.3634%
  （fail3 345,176→271,446）、loto6 20口 47.3279%→47.6686%。
  loto7 11口・loto6 11〜13口は出荷構造が exact 1-swap 局所最適のため据置。
- 導出手法を変更: 従来の焼きなましは1評価が exact DP（20口で約1.3秒）で
  試行回数を稼げなかった。numpy の共通乱数モンテカルロ（1評価ミリ秒）で
  広く探索し、差分 exact による 1-swap polish で仕上げる二段構えにした。
  polish は「対象の口を除いた全口が fail」となる抽選集合のみ再評価するため
  exact のまま高速。
- 採用条件は fail3 の厳密改善に加え、loto7 は exact 入賞確率の非悪化。
  入賞は `本数字>=3 かつ 本数字+ボーナス>=4` と同値で、C(37,7) の直接列挙で
  厳密計算した（loto6 は入賞 <=> any3 なので fail3 の改善で足りる）。
- v5.11 の「N口テーブル == N+1口テーブルの縮約」という関係は、独立再探索で
  より良い構造が見つかったため失われた。該当テストは削除し、
  shrink_next_portfolio 自体は導出プリミティブとして残した。
...
- hitprob 組数上限を 15→20 に拡張（拡張プラスモード第2弾、v5.9 と同じ
  best-found 方式・最適性証明なし）。exact any3: loto6 20口 47.33% /
  loto7 20口 96.65%。
- 11口の2ケースを追加シードで微改善（loto6 11口 28.3827→28.3878% /
  loto7 11口 82.5426→82.5429%）。他8ケースは4独立シードが既存 best を
  超えられず、best-found の信頼度を補強。
- optimize_hitprob_extended.anneal の最終山登りの時間チェックをスイープ
...
_PRECOMPUTED_BEST_FOUND_PORTFOLIOS = {
    ...
    # fail3=3937272, exact any3=35.4170% (best-found)
    ('loto6', 14): (
        (11, 12, 22, 25, 30, 31),
        (5, 6, 15, 32, 36, 37),
        (10, 16, 20, 23, 25, 35),
    ...
    # fail3=3808860, exact any3=37.5234% (best-found)
    ('loto6', 15): (
        (11, 16, 18, 23, 32, 33),
        (5, 14, 21, 22, 30, 39),
        (7, 12, 16, 28, 29, 41),
    ...
    # fail3=3682945, exact any3=39.5887% (best-found)
    ('loto6', 16): (
        (8, 9, 26, 27, 29, 32),
        (4, 6, 23, 25, 34, 41),
        (1, 4, 11, 35, 38, 40),
    ...
    # fail3=3559713, exact any3=41.6101% (best-found)
    ('loto6', 17): (
        (2, 3, 12, 34, 35, 39),
        (3, 11, 22, 23, 32, 38),
        (13, 14, 19, 24, 27, 31),
    ...
    # fail3=3434680, exact any3=43.6610% (best-found)
    ('loto6', 18): (
        (1, 6, 26, 28, 33, 37),
        (9, 12, 17, 24, 32, 39),
        (7, 16, 24, 27, 29, 30),
    ...
    # fail3=3312775, exact any3=45.6606% (best-found)
    ('loto6', 19): (
        (11, 15, 16, 19, 32, 39),
        (1, 3, 16, 34, 37, 40),
        (7, 13, 18, 24, 30, 39),
    ...
    # fail3=3190362, exact any3=47.6686% (best-found)
    ('loto6', 20): (
        (2, 8, 23, 27, 31, 40),
        (9, 11, 26, 27, 29, 30),
        (6, 20, 21, 24, 29, 31),
    ...
    # fail3=1797292, exact any3=82.5429% (best-found)
    ('loto7', 11): (
        (5, 12, 14, 22, 23, 25, 32),
        (3, 11, 18, 20, 21, 27, 36),
        (9, 10, 14, 16, 21, 31, 33),
    ...
    # fail3=1537623, exact any3=85.0651% (best-found)
    ('loto7', 12): (
        (11, 12, 15, 18, 20, 27, 29),
        (3, 10, 16, 17, 26, 27, 33),
        (5, 8, 12, 13, 22, 36, 37),
        (1, 2, 14, 24, 25, 30, 37),
        (7, 13, 16, 20, 21, 25, 32),
    ...
    # fail3=1286253, exact any3=87.5066% (best-found)
    ('loto7', 13): (
        (3, 9, 19, 20, 23, 26, 28),
        (2, 10, 16, 17, 23, 29, 31),
        (2, 4, 8, 19, 22, 33, 37),
    ...
    # fail3=1042922, exact any3=89.8701% (best-found)
    ('loto7', 14): (
        (6, 10, 11, 19, 24, 26, 35),
        (4, 6, 20, 23, 25, 27, 29),
        (1, 5, 14, 19, 27, 32, 34),
    ...
    # fail3=809567, exact any3=92.1367% (best-found)
    ('loto7', 15): (
        (2, 7, 8, 14, 30, 31, 37),
        (4, 9, 13, 22, 26, 28, 29),
        (5, 9, 10, 16, 17, 34, 37),
    ...
    # fail3=624359, exact any3=93.9356% (best-found)
    ('loto7', 16): (
        (5, 10, 13, 16, 23, 31, 36),
        (5, 9, 12, 20, 27, 29, 32),
        (8, 10, 15, 17, 20, 28, 37),
    ...
    # fail3=533887, exact any3=94.8144% (best-found)
    ('loto7', 17): (
        (5, 10, 15, 17, 20, 30, 31),
        (10, 11, 16, 18, 23, 25, 28),
        (1, 6, 19, 20, 25, 26, 36),
        (6, 7, 11, 14, 24, 33, 35),
        (2, 7, 12, 16, 27, 30, 37),
        (3, 8, 12, 17, 25, 32, 33),
    ...
    # fail3=439570, exact any3=95.7305% (best-found)
    ('loto7', 18): (
        (8, 9, 16, 18, 23, 27, 29),
        (4, 7, 18, 19, 25, 26, 33),
        (8, 11, 17, 20, 21, 26, 28),
    ...
    # fail3=352783, exact any3=96.5734% (best-found)
    ('loto7', 19): (
        (7, 9, 14, 21, 23, 26, 28),
        (7, 9, 12, 15, 24, 32, 33),
        (6, 12, 16, 20, 21, 27, 29),
    ...
    # fail3=271446, exact any3=97.3634% (best-found)
    ('loto7', 20): (
        (3, 7, 8, 20, 27, 30, 35),
        (9, 13, 16, 20, 21, 24, 29),
        (4, 9, 11, 18, 23, 34, 35),
